...
from entities import VariantSixFineTuneDataset
...
dataset_name = 'c_test.csv'

BEST_MODEL_PATH = 'model/patch_variant_6_finetune_best_model.sav'
...
def predict_test_data(model, testing_generator, device, need_prob=False):
    ...
    model.eval()
    with torch.no_grad():
        for id_batch, url_batch, added_input_list_batch, added_mask_list_batch, removed_input_list_batch, removed_mask_list_batch, label_batch in tqdm(testing_generator):
            added_input_list_batch = added_input_list_batch.to(device)
...
def get_avg_validation_loss(model, validation_generator, loss_function):
    validation_loss = 0
    with torch.no_grad():
        for id_batch, url_batch, added_input_list_batch, added_mask_list_batch, removed_input_list_batch, removed_mask_list_batch, label_batch in validation_generator:
            added_input_list_batch = added_input_list_batch.to(device)
            added_mask_list_batch = added_mask_list_batch.to(device)
            removed_input_list_batch = removed_input_list_batch.to(device)
            removed_mask_list_batch = removed_mask_list_batch.to(device)
...
def train(model, learning_rate, number_of_epochs, training_generator, val_generator,
    ...
    for epoch in range(number_of_epochs):
        ...
        total_loss = 0
        current_batch = 0
        for id_batch, url_batch, added_input_list_batch, added_mask_list_batch, removed_input_list_batch, removed_mask_list_batch, label_batch in training_generator:
            added_input_list_batch = added_input_list_batch.to(device)
            added_mask_list_batch = added_mask_list_batch.to(device)
            removed_input_list_batch = removed_input_list_batch.to(device)
            removed_mask_list_batch = removed_mask_list_batch.to(device)
...
def retrieve_patch_data(all_data, all_label, all_url):
    tokenizer = RobertaTokenizer.from_pretrained("/Users/zyyyyy/Documents/data/c_test/codebert-base")

    print("Preparing tokenizer data...")

    id_to_label = {}
    id_to_url = {}
    id_to_added_input_list = {}
    id_to_added_mask_list = {}
    id_to_removed_input_list = {}
    id_to_removed_mask_list = {}
    ...
    for i, file_list in tqdm(enumerate(all_data)):
        ...
        for count, file in enumerate(file_list):
            ...
            added_code = tokenizer.sep_token + preprocess_variant_1.get_code_version(diff=file, added_version=True)
            deleted_code = tokenizer.sep_token +  preprocess_variant_1.get_code_version(diff=file, added_version=False)

            added_code_list.append(added_code)
            removed_code_list.append(deleted_code)

        # padding
        while len(added_code_list) < 5:
            added_code_list.append(tokenizer.sep_token)

        while len(removed_code_list) < 5:
            removed_code_list.append(tokenizer.sep_token)

        input_ids_list, mask_list = get_input_and_mask(tokenizer, added_code_list)
        id_to_added_input_list[i] = input_ids_list
        id_to_added_mask_list[i] = mask_list

        input_ids_list, mask_list = get_input_and_mask(tokenizer, removed_code_list)
        id_to_removed_input_list[i] = input_ids_list
        id_to_removed_mask_list[i] = mask_list

        id_to_label[i] = all_label[i]
        id_to_url[i] = all_url[i]

    return id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list, id_to_label, id_to_url
...
def get_data():
    ...
    patch_train, patch_val, patch_test_Cp, patch_test_C = [], [], [], []
    label_train, label_val, label_test_Cp, label_test_C = [], [], [], []
    url_train, url_val, url_test_Cp, url_test_C = [], [], [], []
    ...
    for key in url_to_diff.keys():
        ...
        if partition == 'train':
            ...
            if pl == 'C++':
                patch_test_Cp.append(diff)
                label_test_Cp.append(label)
                url_test_Cp.append(url)
            elif pl == 'C':
                patch_test_C.append(diff)
    ...
    patch_data = {'train': patch_train, 'val': patch_val,
                  'test_Cp': patch_test_Cp, 'test_C': patch_test_C}
...
def do_train():
    ...
    for i in range(len(patch_data['test_Cp'])):
        test_Cp_ids.append(index)
        index += 1

    for i in range(len(patch_data['test_C'])):
        test_C_ids.append(index)
        index += 1

    all_data = patch_data['train'] + patch_data['val'] + patch_data['test_Cp'] + patch_data['test_C']
    all_label = label_data['train'] + label_data['val'] + label_data['test_Cp'] + label_data['test_C']
    all_url = url_data['train'] + url_data['val'] + url_data['test_Cp'] + url_data['test_C']

    print("Preparing commit patch data...")
    id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list, id_to_label, id_to_url \
        = retrieve_patch_data(all_data, all_label, all_url)
    print("Finish preparing commit patch data")

    training_set = VariantSixFineTuneDataset(train_ids, id_to_label, id_to_url, id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list)
    val_set = VariantSixFineTuneDataset(val_ids, id_to_label, id_to_url, id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list)
    test_Cp_set = VariantSixFineTuneDataset(test_Cp_ids, id_to_label, id_to_url, id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list)
    test_C_set = VariantSixFineTuneDataset(test_C_ids, id_to_label, id_to_url, id_to_added_input_list, id_to_added_mask_list, id_to_removed_input_list, id_to_removed_mask_list)
